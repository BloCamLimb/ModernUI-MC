...
public class ExtendedGuiGraphics {
    ...
    private void drawRoundRect(float left, float top, float right, float bottom,
        ...
        if ((pose.properties() & Matrix4f.PROPERTY_TRANSLATION) != 0) {
            ...
            buffer.vertex(pose, left - outset, top - outset, depth).color(mColorTL).endVertex();
            buffer.vertex(pose, left - outset, bottom + outset, depth).color(mColorBL).endVertex();
            buffer.vertex(pose, right + outset, bottom + outset, depth).color(mColorBR).endVertex();
            buffer.vertex(pose, right + outset, top - outset, depth).color(mColorTR).endVertex();
            ...
            shader.safeGetUniform("u_Radii")
                    .set(radius, thickness);
            var buffer = bufferSource.getBuffer(GuiRenderType.roundRect());
            buffer.vertex(left - outset, top - outset, depth).color(mColorTL).endVertex();
            buffer.vertex(left - outset, bottom + outset, depth).color(mColorBL).endVertex();
            buffer.vertex(right + outset, bottom + outset, depth).color(mColorBR).endVertex();
            buffer.vertex(right + outset, top - outset, depth).color(mColorTR).endVertex();
