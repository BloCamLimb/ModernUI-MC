...
public final class UIManager implements LifecycleOwner {
     ...
     */
    @MainThread
    void open(@Nonnull Fragment fragment, @Nullable UICallback callback) {
    ...
    @MainThread
    void open(LocalPlayer p, AbstractContainerMenu menu, @Nonnull ResourceLocation key) {
    ...
    void onPostKeyInput(@Nonnull InputEvent.KeyInputEvent event) {
        ...
        if (event.getAction() == GLFW_PRESS) {
            switch (event.getKey()) {
                ...
                //case GLFW_KEY_H -> open(new TestFragment(), new UICallback());
                case GLFW_KEY_J -> open(new TestPauseFragment(), new UICallback());
                case GLFW_KEY_U -> open(new TestListFragment(), new UICallback());
