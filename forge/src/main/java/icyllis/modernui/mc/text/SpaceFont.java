...
public class SpaceFont implements Font {
    ...
    @Nonnull
    public static SpaceFont create(JsonObject metadata, ResourceLocation fontName) {
        JsonObject advances = GsonHelper.getAsJsonObject(metadata, "advances");
        var map = new Int2FloatOpenHashMap();
        for (var entry : advances.entrySet()) {
            int[] codePoints = entry.getKey().codePoints().toArray();
            if (codePoints.length != 1) {
                throw new JsonParseException("Expected single codepoint, got " + Arrays.toString(codePoints));
            }
            float advance = GsonHelper.convertToFloat(entry.getValue(), "advance");
            map.put(codePoints[0], advance);
        }
